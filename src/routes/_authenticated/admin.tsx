...
function AdminPanelPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <PageHeader
        title="Control Panel"
        description={`'''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''\n                                        \n                                            \n                                            CRITICAL: FIX SUPPLIER LEDGER ONLY. DO NOT CHANGE ANY OTHER FEATURE.\n\nProblem:\n\nSupplier Ledger is blank/not showing for ALL shops, including Hafiz Mart 023 and AL-TAJ Mart 025.\n\nI have already fixed the database RPC \`get_supplier_ledger()\` for:\n\n1. ambiguous \`id\` reference\n\n2. tenant fallback\n\nDo NOT rewrite or replace this RPC unless your read-only investigation proves it is still the root cause.\n\nREAD-ONLY INVESTIGATION FIRST:\n\n1. Inspect:\n\n   - src/routes/_authenticated/suppliers.$id.tsx\n\n   - src/routes/_authenticated/suppliers.index.tsx\n\n   - src/lib/supplier-ledger.ts\n\n   - the current database function \`get_supplier_ledger\`\n\n   - \`current_tenant_id()\`\n\n2. Trace the complete flow:\n\n   Supplier list → Ledger button → supplier ID → RPC call → RPC response → ledger rows.\n\n3. Do NOT make any changes during investigation.\n\n4. Identify the exact reason why the UI receives/displays no ledger rows.\n\n5. Check browser/runtime console errors and Supabase RPC errors if available.\n\n6. Verify the RPC using a REAL authenticated shop session context, not an unauthenticated SQL test.\n\nIMPORTANT:\n\nThe supplier ledger page currently calls:\n\nsupabase.rpc(\"get_supplier_ledger\", { p_supplier_id: id })\n\nand maps:\n\noccurred_at → date\n\nentry_type → type\n\nreference → ref\n\nnote → note\n\ndebit → debit\n\ncredit → credit\n\nsource_data → data\n\nDo not change this mapping unless the actual RPC response schema proves it is wrong.\n\nEXPECTED BEHAVIOR:\n\nFor any supplier with purchases/payments/returns, the ledger must show:\n\n- Purchase = Debit\n\n- On-invoice paid amount = Credit\n\n- Supplier payment = Credit\n\n- Purchase return = Credit\n\n- Running balance must calculate correctly.\n\nMULTI-TENANT SAFETY:\n\n- Never remove tenant isolation.\n\n- Never use data from another shop.\n\n- Supplier ID must belong to the currently selected/active tenant.\n\n- Do not make the RPC return all suppliers or all shops.\n\n- Do not bypass RLS/security except where the existing SECURITY DEFINER design already requires it.\n\nDATA SAFETY:\n\n- DO NOT modify/delete/update any existing supplier, purchase, purchase return, payment, sales, inventory, cash-flow or ledger data.\n\n- DO NOT change any unrelated RPC.\n\n- DO NOT modify dashboard, POS, purchase, return, inventory, cash flow or customer ledger.\n\n- DO NOT perform migrations unrelated to Supplier Ledger.\n\nFIX:\n\nAfter the exact root cause is confirmed, make the MINIMUM possible change required to make Supplier Ledger work for all shops.\n\nVALIDATION REQUIRED:\n\nTest at least:\n\n1. Hafiz Mart 023 supplier with existing purchases.\n\n2. AL-TAJ Mart 025 supplier with existing purchases.\n\n3. A supplier with only a payment.\n\n4. A supplier with a purchase return.\n\n5. A supplier with no transactions → should correctly show \"No transactions yet\".\n\nThen verify:\n\n- no cross-shop data\n\n- no duplicate ledger entries\n\n- no console/RPC errors\n\n- TypeScript/build remains clean.\n\nBefore finishing, report:\n\n1. EXACT root cause\n\n2. EXACT files/functions changed\n\n3. What was NOT changed\n\n4. Tests performed and results\n\nDo not refactor. Do not optimize. Do not redesign. Do not touch anything outside Supplier Ledger.`}
