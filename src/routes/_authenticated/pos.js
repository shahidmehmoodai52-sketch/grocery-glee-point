...
import { deriveDigitalCashBackSummary, normalizePaymentAllocations, normalizePaymentMethodValue, sumPaymentAllocations, } from "@/lib/pos-payments";
import { Receipt, printInvoiceDirect } from "@/components/receipt";
import { fetchAll } from "@/lib/supabase-page";
import { ShiftBanner } from "@/components/shift-banner";
import { offlineFirst, cacheProducts, cacheCustomers, cacheProductBarcodes, completeSaleOfflineAware, cacheSuppliers, insertOfflineAware, } from "@/lib/offline/pos";
...
const POS_CASH_ACCOUNTS_QUERY_KEY = ["cash-accounts", "pos-payment"];
async function fetchExpensePersons() {
    return offlineFirst(async () => {
...
async function fetchActiveCashAccounts() {
    return offlineFirst(async () => {
        ...
        if (error)
            throw error;
        return data ?? [];
    }, async () => (await offlineDb().cash_accounts.toArray())
...
function toStoredSplitPayments(allocations, targetPaid) {
...
function parsePaymentMethod(methodValue, paidValue) {
    ...
    if (!rows.length)
        return [{ method: "cash", amount: paid }];
    return rows;
}
...
async function searchProductsOnline(q) {
    const like = `%${q}%`;
    ...
    if (firstError)
        throw firstError;
    const matchedBarcodesByProduct = {};
    for (const row of extraBarcodeRes.data ?? []) {
        if (!matchedBarcodesByProduct[row.product_id])
            matchedBarcodesByProduct[row.product_id] = [];
    ...
    const extraProductsRes = extraIds.length
        ? await supabase
            ...
            .limit(24)
        : { data: [], error: null };
    if (extraProductsRes.error)
        throw extraProductsRes.error;
...
function POSPage() {
    ...
    const [tabs, setTabs] = usePersistentState("pos-tabs", [newTab(1)]);
    const [active, setActive] = usePersistentState("pos-active-tab", tabs[0]?.id ?? "");
    ...
    const [, setLastInvoice] = useState(null);
    const [reprintOpen, setReprintOpen] = useState(false);
    const [reprintView, setReprintView] = useState(null);
    ...
    const undoWindowMin = Math.max(1, Number(settings?.undo_window_minutes ?? 5));
    const [showCost, setShowCost] = useState(false);
    const [showStaff, setShowStaff] = useState(false);
    const [showProfit, setShowProfit] = useState(false);
    const [highlight, setHighlight] = useState(0);
    const [cartCursor, setCartCursor] = useState(-1);
    const cartRowRefs = useRef([]);
    const searchRowRefs = useRef([]);
    ...
    const [undoReason, setUndoReason] = useState(UNDO_REASONS[0]);
    const [undoReasonNote, setUndoReasonNote] = useState("");
    ...
    const saveQuickCustomer = async () => {
        ...
        try {
            ...
            toast.success(data._offline_pending
    ...
    const selectQuickAddMatch = async (product) => {
        addProduct(product);
    ...
    const { data: quickAddSuppliers = [] } = useQuery({
        ...
        queryFn: () => offlineFirst(async () => {
            ...
            if (error)
                throw error;
            return data ?? [];
        }, async () => (await offlineDb().suppliers.toArray()).sort((a, b) => String(a.name ?? "").localeCompare(String(b.name ?? ""))), (rows) => cacheSuppliers(rows)),
    });
    const { data: quickAddCategories = [] } = useQuery({
        ...
        queryFn: () => offlineFirst(async () => {
            ...
            const set = new Set();
            (await offlineDb().products.toArray()).forEach((r) => {
                if (r.category)
                    set.add(String(r.category));
            });
            return Array.from(set).sort();
    ...
    const { data: cashAccountOptions = [] } = useQuery({
        ...
        queryFn: () => offlineFirst(fetchActiveCashAccounts, async () => (await offlineDb().cash_accounts.toArray()).filter((a) => a.is_active !== false), async (rows) => {
            await offlineDb().cash_accounts.bulkPut(rows);
        }),
    });
    const { data: products = [], isLoading: productsLoading } = useQuery({
        queryKey: ["products", "active"],
        queryFn: () => offlineFirst(() => fetchAll((from, to) => supabase
            .from("products")
            .select(PRODUCT_COLUMNS)
            .eq("is_active", true)
            .order("name")
            .range(from, to)), async () => (await offlineDb().products.toArray())
            .filter((p) => p.is_active !== false)
            .sort((a, b) => (a.name ?? "").localeCompare(b.name ?? "")), (rows) => cacheProducts(rows)),
        staleTime: 5 * 60 * 1000,
    });
    ...
    const searchableProducts = useMemo(() => (products.length > 0 ? products : remoteProducts), [products, remoteProducts]);
    const { data: extraBarcodes = [] } = useQuery({
        queryKey: ["product_barcodes"],
        queryFn: () => offlineFirst(() => fetchAll((from, to) => supabase.from("product_barcodes").select("id,product_id,barcode").range(from, to)), () => offlineDb().product_barcodes.toArray(), (rows) => cacheProductBarcodes(rows)),
    });
    ...
    const barcodesByProduct = useMemo(() => {
        ...
        extraBarcodes.forEach((b) => {
            if (!m[b.product_id])
                m[b.product_id] = [];
            if (!m[b.product_id].includes(b.barcode))
                m[b.product_id].push(b.barcode);
        });
        return m;
            ...
            try {
                for (let i = 0; i < itemCodeLookupBarcodes.length; i += 500) {
                    ...
                    (data ?? []).forEach((row) => {
                        ...
                        if (barcode && itemCode && itemCode !== barcode)
                            map[barcode] = itemCode;
                    });
                }
            }
            catch {
    ...
    const itemCodeByBarcode = useMemo(() => {
        const m = { ...libraryItemCodes };
        searchableProducts.forEach((p) => {
            const sku = cleanItemCode(p.sku);
            if (!sku)
                return;
            if (p.barcode)
                m[String(p.barcode)] = sku;
    ...
    const { data: customers = [] } = useQuery({
        ...
        queryFn: () => offlineFirst(async () => {
            ...
            if (error)
                throw error;
            return data ?? [];
        }, async () => (await offlineDb().customers.toArray()).sort((a, b) => (a.name ?? "").localeCompare(b.name ?? "")), (rows) => cacheCustomers(rows)),
    });
    ...
    const filtered = useMemo(() => {
        ...
        for (const p of searchableProducts) {
            ...
            let s = -1;
            if (sku === q || bcs.includes(q))
                s = 0;
            else if (sku.startsWith(q) || bcs.some((b) => b.startsWith(q)))
                s = 1;
                ...
                s = 4;
            else if (sku.includes(q) || bcs.some((b) => b.includes(q)))
                s = 5;
    ...
    useEffect(() => {
        ...
        if (cartCursor >= tab.items.length)
    ...
    const setTab = (patch) => setTabs((ts) => ts.map((t) => (t.id === active ? { ...t, ...patch } : t)));
    const addProduct = (p) => {
    ...
    const removeLine = (idx) => setTab({ items: tab.items.filter((_, i) => i !== idx) });
    const subtotal = tab.items.reduce((s, i) => s + Math.max(Number(i.qty) * Number(i.price) - Number(i.disc || 0), 0), 0);
    ...
    const paymentRows = Array.isArray(tab.payments) && tab.payments.length
        ? tab.payments
    ...
    const setPrimaryPaymentMethod = (method) => {
    ...
    const accountNameById = (accountId) => {
        const account = (cashAccountOptions ?? []).find((a) => a.id === accountId);
    ...
    const setDigitalAccount = (accountId) => {
    ...
    const setDigitalCashBackAccount = (accountId) => {
        setTab({
    ...
    const setDigitalAmount = (value) => {
        const rowMethod = paymentRows[0]?.method && normalizePaymentMethodValue(tab.payment_method) === "digital"
    ...
    // ---- Held bills ----
    const holdBillsEnabled = !!settings?.ops_hold_bills_enabled;
    const { data: heldBills = [], refetch: refetchHeld } = useQuery({
        ...
        queryFn: () => offlineFirst(async () => {
            ...
            if (error)
                throw error;
            return data ?? [];
        }, async () => (await offlineDb()
            .held_bills.where("status")
            .equals("held")
            .reverse()
            .sortBy("created_at")), async (rows) => {
            try {
                await offlineDb().held_bills.bulkPut(rows.map((r) => ({ ...r, status: "held" })));
            }
            catch { }
        }),
        staleTime: 30000,
    });
    const restorePayloadIntoNewTab = (payload, labelPrefix = "↺") => {
        ...
        const restoredItems = payload.items.map((i) => {
    ...
    const resumeHeld = async (id) => {
        ...
        if (isOfflineNow()) {
            ...
            if (!local)
                return toast.error("Held bill not available offline");
        ...
        restorePayloadIntoNewTab(payload, "↺");
        setHeldOpen(false);
        refetchHeld();
    ...
    const discardHeld = async (id) => {
        ...
        if (isOfflineNow()) {
            ...
            const { error } = await supabase.rpc("discard_held_bill", {
                _id: id,
                _reason: null,
            });
            if (error)
    ...
    const holdCurrent = async () => {
        ...
        if (!holdBillsEnabled)
            return toast.error("Hold bills is disabled in Settings");
        setHolding(true);
        try {
            const payload = {
                items: tab.items,
    ...
    const handleSale = async () => {
        if (saleLockRef.current)
            return; // already submitting — ignore repeat Enter/click
    ...
    const handleSaleInner = async () => {
        ...
        if (isDigitalMode) {
            const hasAccounts = (cashAccountOptions ?? []).some((a) => a.type !== "cash");
        ...
        try {
            ...
            if (ids.length) {
                const { data: stockRows } = await supabase
                    .from("products")
                    .select("id,name,stock,allow_negative_stock")
                    .in("id", ids);
    ...
    const doSale = async () => {
        ...
        try {
                ...
                if (isOfflineNow()) {
                    try {
                        ...
                        await offlineDb().transaction("rw", offlineDb().sales, offlineDb().sale_items, offlineDb().products, async () => {
                            ...
                            if (!prev)
                                throw new Error("Invoice not available offline");
                            ...
                            for (const pid of productIds) {
                                ...
                                if (!p)
                                    continue;
                                const baseStock = typeof p.stock === "number"
                                    ? Number(p.stock)
                        ...
                        if (existingQueuedCreate?.table === "complete_sale") {
                            const queuedPayload = existingQueuedCreate.payload?.payload ?? {};
            ...
            const staffPerson = tab.expense_person_id
                ? persons.find((p) => p.id === tab.expense_person_id)
                : null;
            const payload = {
                customer_id: tab.customer_id,
                expense_person_id: tab.expense_person_id,
            ...
            const { sale, offline } = await completeSaleOfflineAware(payload, {
                charge,
            ...
            const customerRow = tab.customer_id
                ? customers.find((c) => c.id === tab.customer_id)
                : null;
            const patchedSale = sale
                ? {
                    ...sale,
                    sale_items: localItems,
                    customer_id: tab.customer_id ?? sale.customer_id ?? null,
                    customers: sale.customers ?? (customerRow ? { name: customerRow.name } : null),
                    expense_person_id: tab.expense_person_id ?? sale.expense_person_id ?? null,
                    expense_persons: sale.expense_persons ?? (staffPerson ? { name: staffPerson.name } : null),
            ...
            const printPromptEnabled = settings?.pos_print_prompt_enabled === true;
    ...
    const confirmUndo = async () => {
        ...
        try {
            ...
            if (isOfflineNow()) {
                ...
                const createRow = queueRows.find((r) => r.client_uuid === undoCandidate.sale_id && r.table === "complete_sale");
                const relatedEditRows = queueRows.filter((r) => r.table === "edit_sale" && r.payload?._sale_id === undoCandidate.sale_id);
                if (createRow?.id != null)
                    await offlineDb()._queue.delete(createRow.id);
                for (const row of relatedEditRows) {
                    if (row.id != null)
                        await offlineDb()._queue.delete(row.id);
                }
    ...
    useEffect(() => {
        const onKey = (e) => {
            ...
            const isUndoShortcut = e.key === "F10" ||
    ...
    return (<div className="min-h-full md:h-full flex flex-col md:overflow-hidden">
      <ShiftBanner />
      ...
      <div className="flex items-center gap-2 px-2 py-1 border-b bg-card/60 no-print shrink-0">
        <ScrollArea className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            {tabs.map((t) => (<button key={t.id} onClick={() => setActive(t.id)} className={`group flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs whitespace-nowrap ${t.id === active
                ? "bg-background border-primary/40"
                : "bg-muted/40 text-muted-foreground hover:bg-muted"}`}>
                <ShoppingCart className="h-3 w-3"/>
                <span>{t.name}</span>
                {t.items.length > 0 && (<Badge variant="secondary" className="h-4 px-1 text-[10px]">
                    {t.items.length}
                  </Badge>)}
        ...
        {holdBillsEnabled && (<>
            <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={holdCurrent} disabled={holding || !tab.items.length} title="Hold current bill (park cart)">
              <PauseCircle className="h-3.5 w-3.5 mr-1"/> Hold
            </Button>
            <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={() => setHeldOpen(true)} title="Resume a held bill">
              <Play className="h-3.5 w-3.5 mr-1"/> Held
              {heldBills.length > 0 && (<Badge variant="secondary" className="ml-1 h-4 px-1 text-[10px]">
                  {heldBills.length}
                </Badge>)}
            </Button>
          </>)}
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row overflow-hidden">
        ...
        <main className="relative flex-1 flex flex-col min-h-[55vh] md:min-h-0 bg-background overflow-hidden">
          <div className="relative flex items-center gap-3 px-4 py-3 border-b bg-card no-print">
            ...
            <div className="relative flex-1 min-w-0 max-w-[560px]">
              ...
              <Input ref={searchRef} autoFocus placeholder="🔍  Scan barcode or search product…  (F3)" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Escape") {
                setSearch("");
                setCartCursor(-1);
                return;
            ...
            openQuickAdd(raw);
        }} className={`pl-12 h-14 text-base rounded-xl border-2 shadow-sm transition-all duration-300 ${scanFlash
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              {tab.editing_sale_id && (<Badge className="bg-primary/15 text-primary border border-primary/30 text-[11px] shrink-0 rounded-full">
                  <Pencil className="h-3 w-3 mr-1"/> EDITING {tab.editing_invoice_no ?? ""}
                </Badge>)}
              {tab.restored && !tab.editing_sale_id && (<Badge className="bg-warning text-warning-foreground text-[11px] shrink-0 rounded-full">
                  ↩ RESTORED SALE
                </Badge>)}
              {tab.expense_person_id && (<Badge variant="outline" className="border-warning text-warning text-[11px] shrink-0">
                  Staff purchase
                </Badge>)}
            </div>
            <Button size="sm" variant={showCost ? "secondary" : "ghost"} className="h-9 text-xs shrink-0" onClick={() => setShowCost((v) => !v)}>
          ...
          <div className="flex-1 min-h-0 overflow-auto bg-white dark:bg-background">
            <table className="w-full text-sm border-collapse [&_td]:border [&_th]:border [&_td]:border-border [&_th]:border-border">
              <thead className="sticky top-0 z-10 bg-primary text-primary-foreground text-[11px] uppercase tracking-wide">
                <tr>
                  ...
                  {showCost && (<th className="px-2 py-2 text-right w-24 no-print" title="Purchase rate (internal)">
                      P.Rate
                    </th>)}
              ...
              <tbody>
                {tab.items.length === 0 && !search.trim() && (<tr>
            ...
            return (<tr key={idx} ref={(el) => {
                    cartRowRefs.current[idx] = el;
                }} onClick={() => setCartCursor(idx)} className={`${zebra} hover:bg-amber-100/60 dark:hover:bg-muted/40 ${isCursor ? "ring-2 ring-inset ring-primary bg-primary/5" : ""}`}>
                      ...
                      <td className="px-2 py-1">
                        ...
                        {subline && (<div className="text-[11px] text-muted-foreground truncate">
                            {subline}
                          </div>)}
                      </td>
                      <td className="px-2 py-1 text-right tabular-nums">
                        {stockNum !== null ? (<span className={`text-sm font-semibold ${stockNum > 0 ? "text-foreground" : "text-destructive"}`}>
                            {fmtQty(stockNum)}
                            {p?.unit ? (<span className="text-[10px] text-muted-foreground ml-0.5">
                                {p.unit}
                              </span>) : null}
                          </span>) : (<span className="text-muted-foreground">—</span>)}
                      </td>
                      {showCost && (<td className="px-2 py-1 text-right font-mono text-muted-foreground no-print">
                          {fmtMoney(it.cost, sym)}
                        </td>)}
                      <td className="p-0">
                        <EditableNumCell active={editing?.idx === idx && editing.field === "price"} value={it.price} step="0.01" display={fmtMoney(it.price, sym)} onActivate={() => setEditing({ idx, field: "price" })} onCommit={(v) => {
                    ...
                    searchRef.current?.focus();
                      ...
                      <td className="p-0">
                        <EditableNumCell active={editing?.idx === idx && editing.field === "qty"} value={it.qty} step="0.001" display={fmtQty(it.qty)} onActivate={() => setEditing({ idx, field: "qty" })} onCommit={(v) => {
                    ...
                    setTimeout(() => searchRef.current?.focus(), 0);
                      ...
                      <td className="p-0">
                        <EditableNumCell active={editing?.idx === idx && editing.field === "disc"} value={it.disc} step="0.01" min={0} display={fmtMoney(it.disc, sym)} onActivate={() => setEditing({ idx, field: "disc" })} onCommit={(v) => {
          ...
          {search.trim() && (<div className="absolute left-4 right-4 top-[64px] z-40 rounded-xl border border-primary/30 bg-card shadow-2xl overflow-hidden">
              <div className="max-h-[60vh] overflow-auto">
                {filtered.length > 0 ? (<table className="w-full text-sm border-collapse">
                    <thead className="sticky top-0 z-10 bg-primary text-primary-foreground text-[11px] uppercase tracking-wide">
                      <tr>
                    ...
                    return (<tr key={`search-${p.id}`} ref={(el) => {
                            ...
                            {showCost && (<td className="px-2 py-1.5 text-right font-mono text-muted-foreground">
                                {fmtMoney(pRate, sym)}
                              </td>)}
                    ...
                    {productsLoading || remoteProductsLoading ? (<span className="inline-flex items-center gap-2 text-muted-foreground text-sm">
                        <Loader2 className="h-4 w-4 animate-spin"/> Loading products…
                      </span>) : (<span className="inline-flex items-center gap-3 text-sm">
        ...
        <aside className="w-full md:w-[320px] lg:w-[360px] xl:w-[380px] shrink-0 border-t md:border-t-0 md:border-l bg-card flex flex-col min-h-0 max-h-[70vh] md:max-h-[calc(100vh-8.5rem)] overflow-hidden no-print">
          ...
          <div className="p-2 border-b space-y-1.5 shrink-0">
            <div>
              <div className="flex items-center justify-between">
                ...
                {tab.customer_id && (<Link to="/customers/$id" params={{ id: tab.customer_id }} className="text-xs text-primary hover:underline">
                    View ledger →
                  </Link>)}
              </div>
              <div className="flex items-center gap-1.5 mt-1">
                <Select value={tab.customer_id ?? "walkin"} onValueChange={(v) => {
            ...
            setTimeout(() => searchRef.current?.focus(), 0);
                ...
                <Button type="button" size="sm" variant="outline" className="h-9 w-9 shrink-0" title="Quick add customer" onClick={() => setQuickAddCustomerOpen(true)}>
                  <Plus className="h-4 w-4"/>
                </Button>
              </div>
              {(() => {
            const c = tab.customer_id
                ? customers.find((x) => x.id === tab.customer_id)
                : null;
            const bal = c ? Number(c.balance ?? 0) : 0;
            if (!c)
                return null;
            return (<div className="flex items-center gap-1.5 mt-2 flex-wrap">
                    {tab.payment_method === "credit" && (<Badge className="bg-warning text-warning-foreground text-[10px] rounded-full px-2">
                        CREDIT
                      </Badge>)}
                    {bal > 0 && (<Badge variant="outline" className="border-warning text-warning text-[11px]">
            ...
            {(showStaff || tab.expense_person_id) && (<div>
                <Label className="text-xs text-muted-foreground">Staff / Owner purchase</Label>
                <Select value={tab.expense_person_id ?? "none"} onValueChange={(v) => {
                ...
                setTimeout(() => searchRef.current?.focus(), 0);
                  ...
                  <SelectContent>
                    ...
                    {persons.map((p) => (<SelectItem key={p.id} value={p.id}>
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <Button type="button" size="sm" variant={tab.expense_person_id || showStaff ? "secondary" : "ghost"} className="h-6 text-[11px] px-2" title="Charge this bill to a staff/owner expense ledger" onClick={() => {
            ...
            setTimeout(() => searchRef.current?.focus(), 0);
              ...
              <PaymentMethodGrid value={tab.payment_method} onChange={(v) => {
            setPrimaryPaymentMethod(v);
            setTimeout(() => searchRef.current?.focus(), 0);
        }} sym={sym} total={total} due={due} digitalAccountId={tab.digital_account_id ?? null} onSelectDigitalAccount={setDigitalAccount} cashBackReceived={tab.digital_received_amount ?? ""} cashBackAmount={digitalCashBackAmount} onSelectCashBackAccount={setDigitalCashBackAccount} onCashBackReceivedChange={(v) => setTab({ digital_received_amount: v })} expensePersonId={tab.expense_person_id} onSelectStaff={(id) => {
            ...
            setShowStaff(!!id);
              ...
              <div className="mt-1 space-y-1.5">
                {paymentRows.map((payment, idx) => (<div key={`${payment.method}-${idx}`} className="flex items-center gap-1.5">
                    <PaymentMethodSelect value={payment.method} onChange={(v) => {
                ...
                updatePaymentRow(idx, { method: normalized });
                    ...
                    <Input type="number" step="0.01" min="0" value={payment.amount} onChange={(e) => updatePaymentRow(idx, { amount: Number(e.target.value || 0) })} className="h-8 w-24 text-right text-sm"/>
                    {paymentRows.length > 1 && (<Button type="button" size="icon" variant="ghost" className="h-8 w-8 shrink-0" onClick={() => removePaymentRow(idx)}>
                        <Trash2 className="h-3.5 w-3.5 text-destructive"/>
                      </Button>)}
                  </div>))}
              </div>
          ...
          <div className="flex-1 min-h-0 overflow-auto px-2 py-1.5 space-y-1 bg-muted/10 flex flex-col">
            ...
            {lineDiscountTotal > 0 && (<Row label="Line discounts" value={`- ${fmtMoney(lineDiscountTotal, sym)}`} muted/>)}

            <div className="flex items-center justify-between text-sm gap-2">
              <span className="text-muted-foreground">Discount</span>
              <div className="flex items-center gap-1.5">
                <div className="relative">
                  <Input type="number" step="0.01" value={tab.discount_pct} onChange={(e) => applyDiscountPct(e.target.value)} placeholder="0" className="h-8 w-14 text-right text-sm pr-5"/>
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">
                    %
                  </span>
                </div>
                <Input type="number" step="0.01" value={tab.discount} onChange={(e) => setTab({ discount: Number(e.target.value), discount_pct: "" })} className="h-8 w-24 text-right text-sm"/>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-sm gap-2">
              ...
              <div className="flex items-center gap-1.5">
                <div className="relative">
                  <Input type="number" step="0.01" value={tab.charge_pct} onChange={(e) => applyChargePct(e.target.value)} placeholder="0" className="h-8 w-14 text-right text-sm pr-5"/>
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">
                    %
                  </span>
                </div>
                <Input type="number" step="0.01" value={tab.charge} onChange={(e) => setTab({ charge: Number(e.target.value), charge_pct: "" })} className="h-8 w-24 text-right text-sm"/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <Input ref={paidRef} type="number" step="0.01" value={tab.paid} onChange={(e) => {
        ...
        }} placeholder={total.toFixed(2)} className="h-9 flex-1 text-sm font-semibold tabular-nums"/>
            ...
            setTimeout(() => searchRef.current?.focus(), 0);
              ...
              <div className="mt-1">
                {due > 0 ? (<div className="rounded bg-destructive/10 border border-destructive/20 px-2 py-1 text-xs text-destructive font-semibold">
                    Due: {fmtMoney(due, sym)}
                  </div>) : (<div className="rounded bg-success/10 border border-success/20 px-2 py-1 text-sm text-success font-bold tabular-nums">
                    Change: {fmtMoney(change, sym)}
                  </div>)}
              </div>
            </div>

            <Input value={tab.note} onChange={(e) => setTab({ note: e.target.value })} placeholder="Note / House #, street…" className="h-7 text-xs"/>
            ...
            (() => {
                ...
                return (<div className="rounded-md border border-dashed bg-background/60 px-2 py-1 text-[11px]">
                    <button type="button" onClick={() => setShowProfit((v) => !v)} className="flex items-center justify-between w-full text-muted-foreground hover:text-foreground">
                      <span>{showProfit ? "Hide" : "Show"} profit</span>
                      {showProfit ? <EyeOff className="h-3 w-3"/> : <Eye className="h-3 w-3"/>}
                    </button>
                    {showProfit && (<div className="flex items-center justify-between pt-1 mt-1 border-t">
          ...
          <div className="p-2 border-t bg-card shrink-0">
            <Button className="w-full h-11 text-base font-bold rounded-xl shadow-md hover:shadow-lg transition-shadow" onClick={handleSale} disabled={submitting}>
      ...
      {/* Reprint browser */}
      <ReprintDialog open={reprintOpen} onOpenChange={setReprintOpen} settings={settings} sym={sym} reprintAuditEnabled={!!settings?.ops_reprint_audit_enabled} onEdit={(s) => loadInvoiceForEdit(s)}/>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="rounded-md border max-h-[60vh] overflow-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {heldBills.length === 0 && (<tr>
                    <td colSpan={6} className="text-center py-6 text-muted-foreground">
                      No held bills
                    </td>
                  </tr>)}
                {heldBills.map((b) => (<tr key={b.id} className="border-t hover:bg-accent/40">
                    <td className="px-3 py-1.5">
                      <span className="inline-flex items-center gap-1.5">
                        {b.payload?.editing_sale_id ? (<span className="inline-flex items-center gap-1 text-primary" title="Paused edit — resume to continue editing invoice">
                            <Clock className="h-3.5 w-3.5"/>
                            <Pencil className="h-3 w-3"/>
                          </span>) : null}
                        <span>{b.label || "Untitled"}</span>
                        {b.payload?.editing_sale_id && (<Badge variant="outline" className="text-[10px] h-4 px-1 border-primary/40 text-primary">
      ...
      <InvoiceDialog invoice={reprintView} settings={settings} onClose={() => setReprintView(null)}/>

      {/* Post-sale print prompt — Enter triggers the default action (Settings > POS). */}
      <PrintPromptDialog sale={printAsk} defaultAction="no" onYes={() => {
            const s = printAsk;
            setPrintAsk(null);
            if (s) {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="relative">
                ...
                <Input className="pl-9" placeholder="Barcode, item code or name" value={quickAddLookup} onChange={(e) => setQuickAddLookup(e.target.value)}/>
              </div>
              {quickAddLookup.trim().length >= 2 && (<div className="mt-2 max-h-48 overflow-auto rounded-md border bg-muted/20 p-2 space-y-2">
                  {quickAddMatchesLoading && (<div className="text-xs text-muted-foreground">Searching…</div>)}
                  {!quickAddMatchesLoading && quickAddMatches.length === 0 && (<div className="text-xs text-muted-foreground">No existing item found</div>)}
                  {(quickAddMatches ?? []).map((product) => (<button key={product.id} type="button" className="flex w-full items-start justify-between rounded-md border border-border bg-background px-3 py-2 text-left shadow-sm transition hover:border-primary hover:bg-accent/70" onClick={() => selectQuickAddMatch(product)}>
                      <div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <Input autoFocus value={quickAdd.name} onChange={(e) => setQuickAdd((q) => ({ ...q, name: e.target.value }))} onKeyDown={(e) => {
            if (e.key === "Enter") {
                e.preventDefault();
            ...
            <div className="col-span-2">
              ...
              <select value={quickAdd.supplier_id || ""} onChange={(e) => setQuickAdd((q) => ({ ...q, supplier_id: e.target.value }))} className="flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50">
                <option value="">— None —</option>
                {quickAddSuppliers.map((s) => (<option key={s.id} value={s.id}>
                    {s.name}
                  </option>))}
              </select>
            </div>
            ...
            <div className="col-span-2">
              ...
              <textarea className="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={quickAdd.barcodes_text} onChange={(e) => setQuickAdd((q) => ({ ...q, barcodes_text: e.target.value }))} placeholder={"8964000000001\n8964000000002"}/>
            </div>
            <div>
              <Label>Category</Label>
              <Input list="quickadd-category-list" placeholder="e.g. Grocery, Drinks" value={quickAdd.category} onChange={(e) => setQuickAdd((q) => ({ ...q, category: e.target.value }))}/>
            ...
            <div>
              ...
              <Input type="number" step="0.01" value={quickAdd.cost_price} onChange={(e) => setQuickAdd((q) => ({ ...q, cost_price: e.target.value }))}/>
            </div>
            <div>
              <Label>Sell price</Label>
              <Input type="number" step="0.01" value={quickAdd.sell_price} onChange={(e) => setQuickAdd((q) => ({ ...q, sell_price: e.target.value }))}/>
            </div>
            <div>
              <Label>Stock</Label>
              <Input type="number" step="0.001" value={quickAdd.stock} onChange={(e) => setQuickAdd((q) => ({ ...q, stock: e.target.value }))}/>
            </div>
            <div>
              <Label>Low-stock alert at</Label>
              <Input type="number" step="0.001" value={quickAdd.low_stock_threshold} onChange={(e) => setQuickAdd((q) => ({ ...q, low_stock_threshold: e.target.value }))}/>
            </div>
            <div>
              <Label>Tax %</Label>
              <Input type="number" step="0.01" value={quickAdd.tax_rate} onChange={(e) => setQuickAdd((q) => ({ ...q, tax_rate: e.target.value }))}/>
            </div>
            <div>
              <Label>Batch #</Label>
            ...
            <div className="col-span-2 flex items-start gap-2 rounded-md border p-3 bg-muted/30">
              <input id="quickadd-allow-neg-stock" type="checkbox" className="mt-1 h-4 w-4" checked={quickAdd.allow_negative_stock} onChange={(e) => setQuickAdd((q) => ({ ...q, allow_negative_stock: e.target.checked }))}/>
      ...
      <Dialog open={undoOpen} onOpenChange={(o) => !undoing && setUndoOpen(o)}>
        <DialogContent className="max-w-md">
          ...
          {undoCandidate && (<div className="space-y-3 text-sm">
              ...
              <div className="rounded-md border bg-muted/40 p-3 space-y-1.5">
                ...
                <Row label="Elapsed" value={`${Math.floor(undoAgeSeconds / 60)}m ${undoAgeSeconds % 60}s of ${undoWindowMin}m window`} muted/>
              </div>
              <div className="space-y-1.5">
                ...
                <Select value={undoReason} onValueChange={setUndoReason}>
                  ...
                  <SelectContent>
                    {UNDO_REASONS.map((r) => (<SelectItem key={r} value={r}>
                        {r}
                      </SelectItem>))}
                  </SelectContent>
                </Select>
                {undoReason === "Other" && (<Textarea rows={2} value={undoReasonNote} onChange={(e) => setUndoReasonNote(e.target.value)} placeholder="Describe reason…"/>)}
              </div>
              {undoExpired && (<p className="text-destructive text-xs">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={confirmUndo} disabled={undoing || undoExpired || !undoCandidate}>
      ...
      <Dialog open={quickAddCustomerOpen} onOpenChange={setQuickAddCustomerOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div>
              <Label>Name</Label>
              <Input autoFocus value={newCustomer.name} onChange={(e) => setNewCustomer((c) => ({ ...c, name: e.target.value }))} onKeyDown={(e) => {
            if (e.key === "Enter") {
                e.preventDefault();
                saveQuickCustomer();
            }
...
function PaymentMethodGrid({ value, onChange, sym, total, due, digitalAccountId, onSelectDigitalAccount, cashBackReceived, cashBackAmount, onSelectCashBackAccount, onCashBackReceivedChange, expensePersonId, onSelectStaff, }) {
    const accQ = useQuery({
        queryKey: POS_CASH_ACCOUNTS_QUERY_KEY,
    ...
    const btn = (active) => `h-9 rounded-lg text-[11px] font-medium transition-all whitespace-nowrap px-1 ${active
    ...
    return (<div className="mt-1.5 space-y-1.5">
      <div className="grid grid-cols-3 gap-1.5">
        ...
        <Popover open={bankOpen} onOpenChange={setBankOpen}>
          <PopoverTrigger asChild>
            <button type="button" disabled={online.length === 0} className={`${btn(bankActive)} leading-tight flex items-center justify-center gap-1 disabled:opacity-50`} title="Bank / other payment accounts from Cash Flow">
          ...
          <PopoverContent align="end" className="w-64 p-2.5 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {online.map((o) => (<button key={o.id} type="button" title={`Receive in ${o.label} (Cash Flow account)`} onClick={() => {
            ...
            <div className="border-t pt-2 space-y-1.5">
              <button type="button" className={`${btn(isCashBack)} w-full text-[11px] h-9 mb-1`} onClick={() => {
            onSelectCashBackAccount(digitalAccountId ?? online[0]?.id ?? null);
        }}>
                Digital + CB
              </button>
              {isCashBack && (<div className="space-y-2 mb-2 bg-muted/30 p-2 rounded-md border border-dashed">
                  <div className="space-y-1">
                    <Label className="text-[10px]">Digital account</Label>
                    <Select value={digitalAccountId ?? ""} onValueChange={(v) => onSelectCashBackAccount(v || null)}>
                      <SelectTrigger className="h-8">
                        <SelectValue placeholder="Select account"/>
                      </SelectTrigger>
                      <SelectContent>
                        {online.map((o) => (<SelectItem key={o.id} value={o.id}>
                            {o.label}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="space-y-1">
                    ...
                    <Input type="number" step="0.01" min="0" className="h-8 text-right" value={cashBackReceived} onChange={(e) => onCashBackReceivedChange(e.target.value)} placeholder={total.toFixed(2)}/>
                  </div>
              ...
              <StaffSelector value={expensePersonId} onSelect={(id) => {
            onSelectStaff(id);
            setBankOpen(false);
        }}/>
            </div>
      ...
      {online.length === 0 && (<p className="text-[10px] leading-tight text-muted-foreground">
...
function StaffSelector({ value, onSelect, }) {
    const { data: persons = [] } = useQuery({
    ...
    return (<div className="grid grid-cols-2 gap-1.5">
      <button type="button" onClick={() => onSelect(null)} className={`h-9 rounded-lg text-[11px] font-medium transition-all px-1 ${!value
            ? "bg-primary text-primary-foreground shadow-sm ring-2 ring-primary/30"
            : "bg-muted/50 text-foreground hover:bg-muted border border-transparent"}`}>
        None
      </button>
      {persons.map((p) => (<button key={p.id} type="button" onClick={() => onSelect(p.id)} className={`h-9 rounded-lg text-[11px] font-medium transition-all px-1 ${value === p.id
                ? "bg-primary text-primary-foreground shadow-sm ring-2 ring-primary/30"
                : "bg-muted/50 text-foreground hover:bg-muted border border-transparent"}`}>
...
function PaymentMethodSelect({ value, onChange, className, }) {
    const accQ = useQuery({
        queryKey: POS_CASH_ACCOUNTS_QUERY_KEY,
        queryFn: fetchActiveCashAccounts,
    });
    const accounts = accQ.data ?? [];
    const online = accounts.filter((a) => a.type !== "cash");
    const options = [
        { value: "cash", label: "Cash" },
        ...online.map((a) => ({ value: String(a.name), label: String(a.name) })),
...
function Row({ label, value, muted }) {
    return (<div className={`flex justify-between text-sm ${muted ? "text-muted-foreground" : ""}`}>
      <span>{label}</span>
...
function Kbd({ label, hint }) {
    return (<div className="flex items-center gap-2 rounded-md border bg-card px-2 py-1.5">
...
function PrintPromptDialog({ sale, defaultAction = "no", onYes, onNo, }) {
    const yesRef = useRef(null);
    const noRef = useRef(null);
    const [focused, setFocused] = useState(defaultAction || "no");
    ...
    return (<Dialog open={!!sale} onOpenChange={(o) => !o && onNo()}>
      <DialogContent className="max-w-xs" onKeyDown={(e) => {
            if (e.key === "Enter") {
                e.preventDefault();
                if (focused === "yes") {
                    doPrint();
        ...
        <DialogFooter className="gap-2">
          ...
          <Button ref={yesRef} variant={focused === "yes" ? "default" : "outline"} onClick={doPrint}>
...
function InvoiceDialog({ invoice, settings, onClose }) {
    if (!invoice)
        return null;
    return (<Dialog open={!!invoice} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
...
function ReprintDialog({ open, onOpenChange, settings, sym, reprintAuditEnabled, onEdit, }) {
    ...
    const { data: serverHits = [] } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (isFinite(asNum) && term !== "")
                filters.push(`total.eq.${asNum}`, `paid.eq.${asNum}`);
    ...
    const { data: sales = [], isFetching } = useQuery({
        ...
        queryFn: () => offlineFirst(async () => {
            ...
            return Promise.all(rows.map(async (r) => ({
                ...r,
                sale_items: r.sale_items ??
                    (await offlineDb().sale_items.where("sale_id").equals(r.id).toArray()),
            })));
        }, async (rows) => {
            try {
                await offlineDb().sales.bulkPut(rows);
                const items = rows.flatMap((r) => r.sale_items ?? []);
                if (items.length)
                    await offlineDb().sale_items.bulkPut(items);
            }
            catch { }
        }),
    ...
    const filtered = useMemo(() => {
        ...
        const local = !lower
            ...
            : sales.filter((s) => {
                ...
                if (String(s.invoice_no ?? "")
                    .toLowerCase()
                    .includes(lower))
                    return true;
                if ((s.customers?.name ?? "").toLowerCase().includes(lower))
                    return true;
                if ((s.payment_method ?? "").toLowerCase().includes(lower))
                    return true;
                if (isNum) {
                    if (Math.abs(Number(s.total) - asNum) < 1)
                        return true;
                    if (Math.abs(Number(s.paid) - asNum) < 1)
                        return true;
                }
                return false;
            });
        ...
        const byId = new Map();
        for (const s of [...local, ...serverHits])
            byId.set(s.id, s);
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <Input autoFocus placeholder="Search by invoice no, customer, or amount (e.g. 250)…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9 h-10"/>
          </div>
          <div className="rounded-md border max-h-[55vh] overflow-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!isFetching && filtered.length === 0 && (<tr>
                    <td colSpan={5} className="text-center py-6 text-muted-foreground">
                      No invoices match.
                    </td>
                  </tr>)}
                {filtered.map((s) => (<tr key={s.id} className="border-t hover:bg-accent/40">
                ...
                {sales.length >= pageSize && (<tr className="border-t">
...
function EditableNumCell({ active, value, step, min, display, onActivate, onCommit, onCancel, }) {
    ...
    if (!active) {
        return (<button type="button" onClick={onActivate} className="h-8 w-full px-2 text-right text-sm tabular-nums hover:bg-accent/50 focus:outline-none focus-visible:ring-1 focus-visible:ring-ring" title="Click to edit">
        {display}
      </button>);
    }
    return (<Input ref={ref} type="number" step={step} min={min} value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={() => {
