...
proper "check your email" screen). Since custom SMTP still isn't set up, the user considered turning
**email confirmation off entirely** for now (Supabase Dashboard →
Authentication → Sign In / Providers → Email → "Confirm email" toggle —
another dashboard-only setting, no MCP tool for it either, confirmed by
checking every table in the `auth` schema — this lives in GoTrue's own
platform config, not anything queryable/settable via SQL) rather than
deal with SMTP immediately. **Status: still pending, not yet done** —
user said to leave it for later (2026-09-14), nothing toggled yet, still
requires confirmation by email exactly as before. Confirmed this needs
no code changes whenever they do flip it: `handleFinishRegister` in
`src/routes/auth.tsx` already branches correctly on whether `signUp()`
returns a session or not, so it works either way depending on that
toggle — no repo action needed once the user does this themselves.
...
Acceptable per the user's own call whenever they do it; revisit if
fake-signups or a "can't reset my password" complaint shows up. This
SMTP task stays pending — re-enable "Confirm email" once SMTP is
actually set up, don't
