DROP FUNCTION IF EXISTS public.admin_shop_invoices(uuid, text, timestamptz, timestamptz, integer, integer);

CREATE OR REPLACE FUNCTION public.admin_shop_analytics(_tenant_id uuid, _from date DEFAULT ((CURRENT_DATE - '30 days'::interval))::date, _to date DEFAULT CURRENT_DATE)
 RETURNS jsonb
 LANGUAGE plpgsql
