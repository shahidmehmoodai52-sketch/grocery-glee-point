...
CREATE OR REPLACE FUNCTION public.edit_sale(_sale_id uuid, _items jsonb)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Reverse staff-purchase expense
  ...
  -- 2) Recompute from new items (mirrors complete_sale logic)
  ...
  IF v_sale.discount > v_subtotal THEN RAISE EXCEPTION 'Discount exceeds new subtotal'; END IF;

  v_total := v_subtotal + COALESCE(v_sale.tax,0) - COALESCE(v_sale.discount,0);
  v_change := GREATEST(COALESCE(v_sale.paid,0) - v_total, 0);
  IF COALESCE(v_sale.paid,0) >= v_total THEN v_status := 'completed'; ELSE v_status := 'credit'; END IF;
  ...
  -- 5) Re-apply credit balance on customer if credit
  IF v_status='credit' AND v_sale.customer_id IS NOT NULL AND (v_total - COALESCE(v_sale.paid,0)) > 0 THEN
    UPDATE public.customers SET balance = balance + (v_total - COALESCE(v_sale.paid,0)) WHERE id = v_sale.customer_id;
  END IF;
