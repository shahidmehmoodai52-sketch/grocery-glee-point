-- Platform admins had no way to see a tenant's business_type anywhere in
-- the admin panel. Thread it through admin_list_tenants() (read-only,
-- additive column) so the shop list can show/filter by it. Return type
-- changes, so this needs an explicit drop first.
--
-- Verified on tillix-migration-test (via a temporary admin_staff grant,
-- removed immediately after): admin_list_tenants() returns business_type
-- correctly for every existing tenant (all 'grocery', as expected).
DROP FUNCTION IF EXISTS public.admin_list_tenants(text, text, integer, integer);

CREATE FUNCTION public.admin_list_tenants(_search text DEFAULT NULL::text, _status text DEFAULT NULL::text, _limit integer DEFAULT NULL::integer, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, name text, slug text, status text, plan text, business_type text, owner_id uuid, owner_name text, owner_email text, member_count integer, product_count integer, sales_count integer, sales_total numeric, subscription_status text, subscription_expires_at timestamp with time zone, created_at timestamp with time zone, last_activity_at timestamp with time zone, total_count bigint, last_login_at timestamp with time zone, plan_max_users integer, plan_max_products integer)
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN QUERY
  SELECT t.id, t.name, t.slug, t.status, t.plan, t.business_type, t.owner_id,
