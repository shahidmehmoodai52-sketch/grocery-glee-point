...
DECLARE
  ...
  v_digital_cash_back_mode BOOLEAN := COALESCE((payload->>'digital_cash_back_mode')::BOOLEAN, false);
  v_digital_received_amount NUMERIC := NULLIF(payload->>'digital_received_amount','')::NUMERIC;
  v_digital_account UUID := NULLIF(payload->>'digital_account_id','')::UUID;
  v_cash_back_amount NUMERIC;
  v_cash_account UUID;
  v_tx_exists BOOLEAN;
BEGIN
  ...
  IF v_digital_cash_back_mode THEN
    IF v_digital_received_amount IS NULL OR v_digital_received_amount < v_total THEN
      RAISE EXCEPTION 'Digital amount received cannot be less than the sale total';
    END IF;
    IF v_digital_account IS NULL THEN
      RAISE EXCEPTION 'Digital payment account is required';
    END IF;
    SELECT true INTO v_ok FROM public.cash_accounts WHERE id = v_digital_account AND tenant_id = v_tenant AND is_active = true;
    IF NOT COALESCE(v_ok,false) THEN RAISE EXCEPTION 'Digital payment account does not belong to current tenant'; END IF;
    v_paid := v_total;
    v_cash_back_amount := GREATEST(COALESCE(v_digital_received_amount, 0) - v_total, 0);
    v_change := 0;
    v_status := 'completed';
  ELSE
    v_cash_back_amount := 0;
  ...
  IF v_digital_cash_back_mode THEN
    SELECT id INTO v_cash_account
      ...
      WHERE tenant_id = v_tenant AND type = 'cash' AND is_active = true
      ORDER BY sort_order, name
      LIMIT 1;

    IF v_cash_account IS NULL THEN
      INSERT INTO public.cash_accounts (tenant_id, name, type, opening_balance, notes, is_active, user_id)
      VALUES (v_tenant, 'Cash', 'cash', 0, 'Auto-created for POS cash-back', true, v_uid)
      RETURNING id INTO v_cash_account;
    END IF;

    IF COALESCE(v_digital_received_amount, 0) > 0 THEN
      SELECT EXISTS (
        SELECT 1
        FROM public.cash_transactions
        WHERE tenant_id = v_tenant
          AND account_id = v_digital_account
          AND category = 'sale_digital_payment'
          AND reference = 'sale:' || v_sale_id::text
      ) INTO v_tx_exists;
      IF NOT v_tx_exists THEN
        INSERT INTO public.cash_transactions (
          ...
          v_digital_account,
          'in',
          v_digital_received_amount,
          CURRENT_DATE,
          'sale_digital_payment',
          'sale:' || v_sale_id::text,
          'Digital payment received for sale ' || COALESCE(v_inv, v_sale_id::text),
          v_uid
        );
      END IF;
    END IF;

    IF COALESCE(v_cash_back_amount, 0) > 0 THEN
      SELECT EXISTS (
        SELECT 1
        FROM public.cash_transactions
        WHERE tenant_id = v_tenant
          AND account_id = v_cash_account
          AND category = 'customer_cash_back'
          AND reference = 'sale_cash_back:' || v_sale_id::text
      ) INTO v_tx_exists;
      IF NOT v_tx_exists THEN
        INSERT INTO public.cash_transactions (
          tenant_id,
          ...
          CURRENT_DATE,
          'customer_cash_back',
          'sale_cash_back:' || v_sale_id::text,
          'Customer cash back for sale ' || COALESCE(v_inv, v_sale_id::text),
          v_uid
        );
      END IF;
    END IF;
