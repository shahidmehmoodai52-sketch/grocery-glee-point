-- Wires the existing (but previously dormant) product_batches/FEFO system
-- into the live complete_purchase() RPC. purchase_items already had
-- optional batch_no/expiry_date/mfg_date columns (added in Sprint 6C) and a
-- trigger (trg_purchase_item_batch) that creates a product_batches row
-- whenever a purchase_item carries batch/expiry data AND the product has
-- track_batches = true — but complete_purchase() never read those fields
-- out of the payload, so the trigger never fired through the normal
-- purchasing flow. This is the pharmacy vertical's core requirement: every
-- pharmacy purchase line needs batch/expiry capture, which then flows
-- automatically into FEFO consumption at sale time via the existing
-- trg_sale_item_fefo trigger — no change to complete_sale is needed.
--
-- Backward compatible: if an item in the payload has no batch_no/
-- expiry_date/mfg_date keys (every existing grocery caller), these extract
-- as NULL exactly as the columns already default to, so complete_purchase's
-- behavior for existing (non-pharmacy) tenants is byte-for-byte unchanged.
--
-- Verified end-to-end on tillix-migration-test: a track_batches=true test
-- product purchased with batch_no/expiry_date via complete_purchase()
-- produced a product_batches row (qty_initial=10), and a subsequent
-- complete_sale() of qty=3 against the same product correctly triggered
-- FEFO consumption (qty_remaining dropped to 7) with zero changes to
-- complete_sale(). get_advisors reported no new issues.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(payload->'items') LOOP
    ...
    INSERT INTO public.purchase_items (tenant_id, purchase_id, product_id, name, qty, cost, line_total, batch_no, expiry_date, mfg_date)
    VALUES (v_tenant, v_id, v_pid, v_item->>'name', v_qty, v_cost, v_qty * v_cost, v_batch_no, v_expiry_date, v_mfg_date);
