...
BEGIN
  ...
  -- Expenses are stored as a plain business date; resolve the period bounds in
  -- Pakistan Time so a PKT day range never pulls in the previous/next day.
  SELECT COALESCE(sum(amount), 0) INTO v_expenses_total
  FROM expenses
  WHERE tenant_id = v_tenant_id
    AND expense_date >= (p_from_date AT TIME ZONE 'Asia/Karachi')::date
    AND expense_date <= (p_to_date AT TIME ZONE 'Asia/Karachi')::date;
