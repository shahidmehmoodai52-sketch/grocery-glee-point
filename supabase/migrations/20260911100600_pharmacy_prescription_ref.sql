-- Lightweight prescription reference on a sale — a number, photo caption,
-- or free-text note tied to the invoice, not a full e-prescription
-- workflow (the business-type research found no evidence real-world
-- Pakistani pharmacy software or regulation expects the latter; a
-- reference field is what actual competitors do). Additive: nullable,
-- no default behavior change for existing (grocery) sales.
--
-- Verified on tillix-migration-test: complete_sale() with a
-- 'prescription_ref' payload key stores it on the sales row correctly;
-- absent (every existing grocery caller) behaves byte-for-byte as before.
